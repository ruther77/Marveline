...
class TestAuditService:
    ...
    async def test_log_action_generic(self, test_db):
    ...
    async def test_log_action_auto_generates_request_id(self, test_db):
    ...
    async def test_log_create(self, test_db):
    ...
    async def test_log_update_with_diff(self, test_db):
        ...
        assert "total_amount_cents" in log.changes
        assert log.changes["total_amount_cents"] == {"before": 10000, "after": 12000}
        assert "notes" in log.changes
        assert "unchanged_field" not in log.changes  # Pas de diff si identique

        assert log.description == "Updated Reservation #456: status, total_amount_cents, notes"

    async def test_log_update_no_changes(self, test_db):
    ...
    async def test_log_delete_soft(self, test_db):
    ...
    async def test_log_delete_hard(self, test_db):
    ...
    async def test_log_read_sensitive(self, test_db):
        """log_read_sensitive() pour conformité RGPD."""
        audit_service = AuditService(test_db)

        log = await audit_service.log_read_sensitive(
            entity_type="Customer",
            entity_id=123,
            tenant_id=1,
            user_id=42,
    ...
    async def test_log_login_success(self, test_db):
        """log_login() avec success=True."""
        audit_service = AuditService(test_db)

        log = await audit_service.log_login(
            user_id=42,
            tenant_id=1,
            ip_address="192.168.1.100",
            user_agent="Mozilla/5.0",
    ...
    async def test_log_login_failed(self, test_db):
    ...
    async def test_log_logout(self, test_db):
        """log_logout() enregistre déconnexion."""
        audit_service = AuditService(test_db)

        log = await audit_service.log_logout(
            user_id=42,
            tenant_id=1,
            ip_address="192.168.1.100",
            user_agent="Mozilla/5.0",
    ...
    async def test_audit_multi_tenant_isolation(self, test_db):
        """Audit logs isolés par tenant_id."""
        audit_service = AuditService(test_db)

        # Logs tenant 1
        await audit_service.log_action(action="CREATE", tenant_id=1, user_id=10)
        await audit_service.log_action(action="UPDATE", tenant_id=1, user_id=10)

        # Logs tenant 2
        await audit_service.log_action(action="CREATE", tenant_id=2, user_id=20)
        await audit_service.log_action(action="DELETE", tenant_id=2, user_id=20)

        test_db.commit()

        # Query tenant 1 uniquement
        logs_tenant1 = test_db.query(AuditLog).filter(AuditLog.tenant_id == 1).all()
        assert len([l for l in logs_tenant1 if l.account_id == 10]) >= 2

        # Query tenant 2 uniquement
        logs_tenant2 = test_db.query(AuditLog).filter(AuditLog.tenant_id == 2).all()
        assert len([l for l in logs_tenant2 if l.account_id == 20]) >= 2

        # Vérifier isolation stricte
        assert all(log.tenant_id == 1 for log in logs_tenant1 if log.account_id == 10)
        assert all(log.tenant_id == 2 for log in logs_tenant2 if log.account_id == 20)
