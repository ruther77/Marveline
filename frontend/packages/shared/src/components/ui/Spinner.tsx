...
export function Skeleton({
  ...
  const animationStyles = {
    pulse: 'animate-pulse',
    wave: 'animate-shimmer bg-gradient-to-r from-dark-600 via-dark-600 to-dark-600 bg-[length:200%_100%]',
    none: '',
  };
...
export function SkeletonCard() {
  return (
    <div className="bg-dark-900 rounded-xl border border-dark-600 p-4 space-y-4">
...
export function SkeletonTable({ rows = 5, cols = 4 }: { rows?: number; cols?: number }) {
  return (
    <div className="border border-dark-600 rounded-lg overflow-hidden">
      <div className="bg-dark-900 px-4 py-4 flex gap-4">
        {Array.from({ length: cols }).map((_, i) => (
          <Skeleton key={i} variant="text" className="h-4 flex-1" />
        ))}
      </div>
      {Array.from({ length: rows }).map((_, i) => (
        <div key={i} className="bg-dark-900 px-4 py-4 flex gap-4 border-t border-dark-600">
