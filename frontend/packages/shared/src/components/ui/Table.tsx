          ...
          <tr>
            {columns.map((column) => (
              <th
                key={column.key}
                className={cn(
                  'px-4 py-4 text-xs font-semibold text-dark-300 uppercase tracking-wider',
                  alignClass[column.align || 'left'],
                  column.sortable && 'cursor-pointer hover:text-dark-50 transition-colors select-none',
                  column.width
                )}
                ...
                >
                  {columns.map((column) => (
                    <td
                      key={column.key}
                      className={cn(
                        'px-4 py-4 text-sm text-dark-200',
                        alignClass[column.align || 'left']
                      )}
                    >
                      {column.render
...
export function Pagination({
  ...
  return (
    <div className="flex items-center justify-between px-4 py-4 bg-dark-900 border-t border-dark-600 rounded-b-lg">
