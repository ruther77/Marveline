...
export function EmptyState({
  ...
  return (
    ...
    >
      ...
      <h3 className={cn('font-semibold text-dark-50 mb-2', sizeStyles.title)}>
        {title}
      </h3>
      {description && (
...
export function ErrorState({ onRetry, statusCode, className }: { onRetry?: () => void; statusCode?: number; className?: string }) {
  const isClientError = statusCode !== undefined && statusCode >= 400 && statusCode < 500;
  const showRetry = !!onRetry && !isClientError;
  return (
    <EmptyState
      type="error"
      title="Une erreur est survenue"
      description="Impossible de charger les données. Veuillez réessayer."
      action={showRetry ? { label: 'Réessayer', onClick: onRetry!, variant: 'secondary' } : undefined}
      className={className}
    />
  );
}
