    ...
    return (
      <div className="relative">
        ...
        <input
          ...
          className={cn(
            'w-full card text-dark-50 placeholder-dark-400',
        ...
        {showClear && localValue && (
          <button
            onClick={handleClear}
            className={cn(
              'absolute inset-y-0 right-0 flex items-center px-4 text-dark-400 hover:text-dark-50 transition-colors'
            )}
  ...
  return (
    <div className="relative">
      ...
      {shouldShowSuggestions && (
        <div className="absolute top-full left-0 right-0 mt-1 card shadow-xl z-50 max-h-64 overflow-y-auto">
          {suggestions.length === 0 ? (
            <div className="px-4 py-4 text-sm text-dark-400">{emptyMessage}</div>
          ) : (
            suggestions.map((suggestion) => (
              <button
                key={suggestion.id}
                onClick={() => handleSelect(suggestion)}
                className="w-full flex items-center gap-4 px-4 py-2.5 text-left hover:bg-dark-600 transition-colors first:rounded-t-lg last:rounded-b-lg"
              >
                {suggestion.icon && (
