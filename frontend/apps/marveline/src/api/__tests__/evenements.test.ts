...
describe('evenementsApi - listEvenements', () => {
  beforeEach(() => { vi.clearAllMocks() })

  it('récupère liste paginée par défaut', async () => {
    vi.mocked(api.get).mockResolvedValue({ items: [mockListItem], total: 1, skip: 0, limit: 20 })

    const result = await evenementsApi.listEvenements()
  ...
  it('applique filtre status et dates', async () => {
    ...
    await evenementsApi.listEvenements({ status: 'incident', date_from: '2026-06-01', date_to: '2026-06-30' })
...
describe('evenementsApi - listIncidents', () => {
  beforeEach(() => { vi.clearAllMocks() })

  it('récupère les incidents via endpoint dédié', async () => {
    vi.mocked(api.get).mockResolvedValue([
      {
        id: 4,
        event_id: 1,
        description: 'Casse vaisselle',
        severity: 'medium',
        sla_hours: 24,
        declared_at: '2026-06-12T10:00:00Z',
        declared_by: 7,
        affected_items: [1],
        actions: [],
      },
    ...
    const incidents = await evenementsApi.listIncidents(1)

    expect(api.get).toHaveBeenCalledWith('/evenements/1/incidents')
    expect(incidents).toHaveLength(1)
    expect(incidents[0].id).toBe(4)
  })
})
...
describe('evenementsApi - close', () => {
  ...
  it('ferme un événement avec notes obligatoires', async () => {
    vi.mocked(api.post).mockResolvedValue({ ...mockFull, status: 'closed' })

    await evenementsApi.close(1, 'Notes de clôture complètes.')

    expect(api.post).toHaveBeenCalledWith('/evenements/1/close', { notes: 'Notes de clôture complètes.' })
  })
})

describe('evenementsApi - reschedule', () => {
  beforeEach(() => { vi.clearAllMocks() })

  it('reprogramme sans forçage par défaut', async () => {
    const mockResponse = { evenement: { ...mockFull, event_date: '2026-06-01' }, conflicts: [] }
    vi.mocked(api.post).mockResolvedValue(mockResponse)

    const result = await evenementsApi.reschedule(1, '2026-06-01')

    expect(api.post).toHaveBeenCalledWith('/evenements/1/reschedule', { new_date: '2026-06-01', force: false })
    expect(result.conflicts).toHaveLength(0)
  })

  it('reprogramme avec forçage', async () => {
    const mockResponse = { evenement: { ...mockFull, event_date: '2026-06-01' }, conflicts: [] }
    vi.mocked(api.post).mockResolvedValue(mockResponse)

    await evenementsApi.reschedule(1, '2026-06-01', true)

    expect(api.post).toHaveBeenCalledWith('/evenements/1/reschedule', { new_date: '2026-06-01', force: true })
  })

  it('retourne les conflits soft sans modifier la date (force=false)', async () => {
    const conflict = { type: 'other_event_same_date', severity: 'soft', message: '1 autre événement', detail: {} }
    const mockResponse = { evenement: mockFull, conflicts: [conflict] }
    vi.mocked(api.post).mockResolvedValue(mockResponse)

    const result = await evenementsApi.reschedule(1, '2026-06-01', false)
...
describe('evenementsApi - getReport', () => {
  beforeEach(() => { vi.clearAllMocks() })

  it('récupère le rapport de clôture', async () => {
    const mockReport = {
      evenement: mockFull,
      reservation: null,
      summary: { total_incidents: 0, resolved_incidents: 0, open_incidents: 0, critical_incidents: 0, total_actions: 0, closed_actions: 0 },
      incidents: [],
      closure_notes: null,
    ...
    const result = await evenementsApi.getReport(1)

    expect(api.get).toHaveBeenCalledWith('/evenements/1/report')
...
describe('evenementsApi - declareIncident', () => {
  ...
  it('déclare un incident sur un événement', async () => {
    ...
    expect(api.post).toHaveBeenCalledWith('/evenements/1/incidents', expect.objectContaining({
      description: 'Table cassée',
      severity: 'high',
      affected_items: [3, 5],
      declared_at: expect.any(String),
    }))
  })
})
