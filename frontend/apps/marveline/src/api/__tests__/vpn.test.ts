...
describe('VPN API - Peers CRUD', () => {
  ...
  it('récupère liste de tous les peers VPN', async () => {
    ...
    expect(api.get).toHaveBeenCalledWith('/vpn/peers')
  ...
  it('récupère un peer par ID', async () => {
    ...
    vi.mocked(api.get).mockResolvedValue(mockPeer)

    const result = await vpnApi.getPeer('peer-1')

    expect(api.get).toHaveBeenCalledWith('/vpn/peers/peer-1')
  ...
  it('crée un nouveau peer VPN', async () => {
    ...
    expect(api.post).toHaveBeenCalledWith('/vpn/peers', newPeer)
  ...
  it('met à jour un peer existant (PATCH partiel)', async () => {
    ...
    expect(api.patch).toHaveBeenCalledWith('/vpn/peers/peer-1', updateData)
  ...
  it('supprime un peer VPN', async () => {
    ...
    expect(api.delete).toHaveBeenCalledWith('/vpn/peers/peer-1')
  })
  ...
  it('effectue rotation des clés WireGuard d\'un peer', async () => {
    ...
    expect(api.post).toHaveBeenCalledWith('/vpn/peers/peer-1/rotate')
  ...
  it('active un peer VPN', async () => {
    ...
    expect(api.post).toHaveBeenCalledWith('/vpn/peers/peer-1/enable')
  ...
  it('désactive un peer VPN', async () => {
    ...
    expect(api.post).toHaveBeenCalledWith('/vpn/peers/peer-1/disable')
...
describe('VPN API - Config & QR', () => {
  ...
  it('récupère config WireGuard d\'un peer (.conf)', async () => {
    ...
    expect(api.get).toHaveBeenCalledWith('/vpn/peers/peer-1/config')
    expect(result.config).toContain('[Interface]')
    expect(result.filename).toBe('peer-1.conf')
  })

  it('récupère QR code d\'un peer (Blob) via fetchBlob', async () => {
    ...
    expect(fetchBlob).toHaveBeenCalledWith('/vpn/peers/peer-1/qrcode')
...
describe('VPN API - Status', () => {
  ...
  it('récupère statut serveur VPN', async () => {
    ...
    expect(api.get).toHaveBeenCalledWith('/vpn/status')
...
describe('VPN API - IP Pools', () => {
  ...
  it('récupère liste des pools IP', async () => {
    ...
    expect(api.get).toHaveBeenCalledWith('/vpn/ip-pools')
  ...
  it('crée un nouveau pool IP', async () => {
    ...
    expect(api.post).toHaveBeenCalledWith('/vpn/ip-pools', newPool)
    expect(result).toEqual(mockResponse)
