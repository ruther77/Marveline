...
describe('Feature Flags - list', () => {
  ...
  it('récupère liste paginée avec paramètres par défaut', async () => {
    ...
    const result = await featureFlagsApi.listFeatureFlags({ skip: 0, limit: 50 })

    expect(api.get).toHaveBeenCalledWith('/features?skip=0&limit=50')
  ...
  it('gère liste vide (0 feature flags)', async () => {
    ...
    const result = await featureFlagsApi.listFeatureFlags()
...
describe('Feature Flags - get', () => {
  ...
  it('récupère un feature flag par ID', async () => {
    ...
    vi.mocked(api.get).mockResolvedValue(mockFlag)

    const result = await featureFlagsApi.get(1)

    expect(api.get).toHaveBeenCalledWith('/features/1')
...
describe('Feature Flags - create', () => {
  ...
  it('crée un nouveau feature flag', async () => {
    ...
    expect(api.post).toHaveBeenCalledWith('/features', newFlag)
...
describe('Feature Flags - update', () => {
  ...
  it('met à jour un feature flag existant (PATCH partiel)', async () => {
    ...
    expect(api.patch).toHaveBeenCalledWith('/features/1', updateData)
...
describe('Feature Flags - toggle', () => {
  ...
  it('active un feature flag (is_enabled=true)', async () => {
    ...
    expect(api.patch).toHaveBeenCalledWith('/features/1', { is_enabled: true })
  ...
  it('désactive un feature flag (is_enabled=false)', async () => {
    ...
    expect(api.patch).toHaveBeenCalledWith('/features/1', { is_enabled: false })
...
describe('Feature Flags - check', () => {
  ...
  it('évalue un feature flag pour un tenant donné', async () => {
    const mockResponse = {
      name: 'pricing_ui',
      enabled: true,
      reason: 'rollout',
    ...
    const result = await featureFlagsApi.check('pricing_ui', 3)

    expect(api.get).toHaveBeenCalledWith('/features/check/pricing_ui?tenant_id=3')
    expect(result).toEqual(mockResponse)
  })
})
