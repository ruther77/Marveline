...
describe('API Keys - list', () => {
  ...
  it('récupère liste paginée avec paramètres par défaut', async () => {
    ...
    const result = await apiKeysApi.listApiKeys({ skip: 0, limit: 50, include_inactive: false })

    expect(api.get).toHaveBeenCalledWith('/api-keys?skip=0&limit=50&include_inactive=false')
  ...
  it('inclut les API keys inactives si include_inactive=true', async () => {
    ...
    await apiKeysApi.listApiKeys({ skip: 0, limit: 50, include_inactive: true })

    expect(api.get).toHaveBeenCalledWith('/api-keys?skip=0&limit=50&include_inactive=true')
  })
...
describe('API Keys - create', () => {
  ...
  it('crée une nouvelle API key et retourne le secret', async () => {
    ...
    expect(api.post).toHaveBeenCalledWith('/api-keys', newKey)
...
describe('API Keys - update', () => {
  ...
  it('met à jour une API key existante (PATCH partiel)', async () => {
    ...
    expect(api.patch).toHaveBeenCalledWith('/api-keys/1', updateData)
...
describe('API Keys - rotate', () => {
  ...
  it('effectue rotation du secret et retourne nouveau secret', async () => {
    ...
    expect(api.post).toHaveBeenCalledWith('/api-keys/1/rotate')
    expect(result.api_key).toBeDefined()
