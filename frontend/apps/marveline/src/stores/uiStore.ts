...
import { fetchAuthCsrfToken } from '@/api/queries/useAuth';
...
export interface UIState {
  ...
  _csrfTimerId: ReturnType<typeof setTimeout> | null;
}
...
export interface UIActions {
  ...
  /** Réutilise le token persisté si disponible, ne fetch que si absent. */
...
// Deduplication — évite double appel concurrent à /auth/csrf
  ...
  persist(
    (set, get) => ({
      ...
      setTheme: (theme) => {
        ...
        // Appliquer data-theme sur <html> pour les CSS variables duales
      ...
      fetchCsrfToken: async () => {
        // Deduplication — si un fetch est déjà en cours, réutiliser la même promesse
        if (_csrfFetchPromise) return _csrfFetchPromise
        _csrfFetchPromise = (async () => {
          try {
            const response = await fetchAuthCsrfToken();
            const { _csrfTimerId: prevTimer } = get()
            if (prevTimer !== null) clearTimeout(prevTimer)
            const timerId = setTimeout(() => get().fetchCsrfToken(), 14 * 60 * 1000)
            set({ csrfToken: response.csrf_token, _csrfTimerId: timerId });
          } catch {
      ...
      ensureCsrfToken: async () => {
        const { csrfToken, _csrfTimerId } = get()
        // Token déjà présent (persisté localStorage) et timer actif → skip le fetch réseau
        if (csrfToken && _csrfTimerId !== null) return
        // Token présent mais pas de timer (page reload) → juste relancer le renouvellement
        if (csrfToken) {
          const timerId = setTimeout(() => get().fetchCsrfToken(), 14 * 60 * 1000)
          set({ _csrfTimerId: timerId })
          return
        }
        // Pas de token → fetch complet
        await get().fetchCsrfToken()
      },

      clearCsrfToken: () => {
        const { _csrfTimerId } = get()
        if (_csrfTimerId !== null) clearTimeout(_csrfTimerId)
        set({ csrfToken: null, _csrfTimerId: null });
      },
    }),
    {
      name: 'marveline-ui',
      ...
      migrate: (persisted: unknown, version: number) => {
        if (version < 2 && persisted && typeof persisted === 'object') {
          const { theme: _ignored, ...rest } = persisted as Record<string, unknown>
          return rest
        }
